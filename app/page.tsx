...
export default function Home(){
 ...
 return <main>
   ...
   <section className="panel"><h2>CEX ↔ DEX bridge</h2><p className="muted">Hummingbot/Gateway supplies normalized CEX order-book and DEX quote connectivity. INSTA-FLOAN retains the final risk, simulation and authorization boundary.</p><div className="stats"><div>BRIDGE<strong>{bridge?.enabled?'ENABLED':'DISABLED'}</strong></div><div>GATEWAY<strong>{bridge?.gatewayConfigured?'CONFIGURED':'NOT CONFIGURED'}</strong></div><div>CEX CONNECTORS<strong>{bridge?.connectors.filter(c=>c.kind==='CEX').length??0}</strong></div><div>DEX CONNECTORS<strong>{bridge?.connectors.filter(c=>c.kind==='DEX').length??0}</strong></div></div>{bridge&&<><div className="row"><b>Networks</b><span>{bridge.activeNetworks.length?bridge.activeNetworks.join(' · '):'No RPC networks configured'}</span></div><div className="row"><b>Execution</b><span>{bridge.liveTradingEnabled&&bridge.executionAuthorized?'AUTHORIZED BY BRIDGE CONFIG':'READ / DISCOVERY ONLY'}</span></div><p className="muted">Credentials are never returned to the browser. Configure exchange credentials inside Hummingbot's encrypted credential store; this UI exposes connector state only.</p></>}</section>
   ...
   <footer>WALLETCONNECT · MEMORY-ONLY PRIVATE-KEY IMPORT · NO SEED PHRASES · CEX API SECRETS STAY IN HUMMINGBOT · LIVE QUOTES READ-ONLY · REAL EXECUTION OFF</footer>
 </main>
}