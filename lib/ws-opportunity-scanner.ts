...
export function findCrossVenueOpportunities(
  books: OrderBook[],
  options: { minNetUsd: number; feeBps?: number; maxAgeMs?: number; maxResults?: number; slippageBps?: number } = {minNetUsd: 0},
): WebSocketOpportunity[] {
  ...
  for (const [symbol, venues] of grouped) {
    for (const buy of venues) for (const sell of venues) {
      ...
      const buyTop = buy.asks[0]
      const sellTop = sell.bids[0]
      if (!buyTop || !sellTop || sellTop[0] <= buyTop[0]) continue
      const quantity = Math.min(buyTop[1], sellTop[1])
      if (quantity <= 0) continue
      const buyVwap = vwap(buy.asks, quantity)
      const sellVwap = vwap(sell.bids, quantity)
      ...
      result.push({
        venueBuy: buy.venue,
        venueSell: sell.venue,
        symbol,
        side: 'BUY_SELL',
        quantity,
        buyVwap: buyVwap.price,
        sellVwap: sellVwap.price,
        grossUsd,
        estimatedCostsUsd,
        netUsd,
        timestamp: Math.min(buy.timestamp, sell.timestamp),
        executable: netUsd >= options.minNetUsd,
        reason: netUsd >= options.minNetUsd ? undefined : 'NET_BELOW_THRESHOLD',
      })
    }
  }
...
export type WsVenueConfig = {
  ...
  parse: (payload: unknown) => {symbol: string; bids?: OrderLevel[]; asks?: OrderLevel[]; sequence?: number} | null
}

export type ScannerSnapshot = {
  books: OrderBook[]
  opportunities: WebSocketOpportunity[]
  updatedAt: number
}
...
export class WebSocketOpportunityScanner {
  ...
  start(onSnapshot: (snapshot: ScannerSnapshot) => void) {
    this.stopped = false
    for (const config of this.configs) this.connect(config, onSnapshot)
  }
  ...
  private connect(config: WsVenueConfig, onSnapshot: (snapshot: ScannerSnapshot) => void) {
    ...
    socket.addEventListener('message', event => {
      try {
        ...
        const books: OrderBook[] = [...this.books.entries()].map(([bookKey, state]) => {
          const [venue, ...symbolParts] = bookKey.split(':')
          return {venue, symbol: symbolParts.join(':'), bids: topLevels(state, 'bids'), asks: topLevels(state, 'asks'), timestamp: state.timestamp, sequence: state.sequence}
        })
        onSnapshot({books, opportunities: findCrossVenueOpportunities(books, this.options), updatedAt: Date.now()})
      } catch {
        // A malformed provider message must not terminate the scanner.
      }
    })
    socket.addEventListener('close', () => {
      if (!this.stopped) setTimeout(() => this.connect(config, onSnapshot), 1_000)
    })
...
export const binanceConfig = (symbols: string[]): WsVenueConfig => ({
  ...
  symbols,
  subscribe: values => ({method: 'SUBSCRIBE', params: values.map(s => `${s.toLowerCase()}@depth@100ms`), id: Date.now()}),
  parse: payload => {
...
export const coinbaseConfig = (symbols: string[]): WsVenueConfig => ({
  ...
  parse: payload => {
    ...
    for (const [side, price, quantity] of data.changes) {
      const level: OrderLevel = [Number(price), Number(quantity)]
